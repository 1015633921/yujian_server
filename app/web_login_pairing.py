...
class WebLoginPairingService:
    ...
    def expires_at() -> str:
        return (
            datetime.now(timezone.utc) + timedelta(seconds=PAIRING_TTL_SECONDS)
    ...
    def create(self) -> dict[str, Any]:
        ...
        with self.connect() as connection:
            connection.execute(
                """
                UPDATE web_login_pairings SET status = 'expired'
            ...
            connection.execute(
                ...
                (id, browser_secret_hash, verification_code_hash, status,
                 confirmed_user_id, confirmed_session_id, failed_confirm_attempts, created_at, expires_at,
                ...
                (
                    ...
                    verification_code_hash(verification_code),
                    created_at,
                    expires_at,
                ),
            )
    ...
    def confirm(
        ...
        with self.connect() as connection:
            cursor = connection.execute(
                ...
                SET status = 'confirmed', confirmed_user_id = ?, confirmed_session_id = ?,
                    confirmed_at = ?
                WHERE id = ? AND verification_code_hash = ? AND status = 'pending'
                  AND confirmed_user_id IS NULL AND failed_confirm_attempts < 5
                  AND expires_at > ?
                """,
                (
                    principal.user_id,
            ...
            if cursor.rowcount == 1:
                row = connection.execute(
                    "SELECT expires_at FROM web_login_pairings WHERE id = ?",
                    (pairing_id,),
                ).fetchone()
                ...
                row = connection.execute(
                    """
                    SELECT verification_code_hash, status, expires_at, failed_confirm_attempts
                    FROM web_login_pairings WHERE id = ?
                    """,
                    (pairing_id,),
                ).fetchone()
                if not row:
                    ...
                    str(row["verification_code_hash"]), verification_code_hash(verification_code)
                ):
                    if str(row["status"]) == "pending" and str(row["expires_at"]) > timestamp:
                        connection.execute(
                            ...
                            SET failed_confirm_attempts = failed_confirm_attempts + 1,
                                status = CASE
                                    WHEN failed_confirm_attempts + 1 >= 5 THEN 'expired'
                                    ELSE status
                                END
                            WHERE id = ? AND status = 'pending' AND expires_at > ?
                            """,
                            (pairing_id, timestamp),
                        )
                    outcome = "not_found"
    ...
    def claim(self, pairing_id: str, browser_secret: str) -> dict[str, Any]:
        ...
        with self.connect() as connection:
            cursor = connection.execute(
                ...
                WHERE id = ? AND browser_secret_hash = ? AND status = 'confirmed'
                  AND confirmed_user_id IS NOT NULL AND expires_at > ?
                """,
            ...
            if cursor.rowcount == 1:
                row = connection.execute(
                    """
                    SELECT confirmed_user_id FROM web_login_pairings
                ...
                session = self.user_session_service.create_in_connection(
                    connection,
                    str(row["confirmed_user_id"]),
                )
                outcome = "claimed"
                data = {
                    "pairing_id": pairing_id,
                    "status": "consumed",
                    "user_id": str(row["confirmed_user_id"]),
                    "session": session,
                }
            else:
                row = connection.execute(
                    """
                    SELECT browser_secret_hash, status, expires_at
                    FROM web_login_pairings WHERE id = ?
                    """,
                    (pairing_id,),
                ).fetchone()
                if not row or not hmac.compare_digest(
                    str(row["browser_secret_hash"]), supplied_hash
                ):
                    outcome = "not_found"
                else:
...
def require_web_login_pairing_bff(
    supplied_token: str | None = Header(default=None, alias=BFF_TOKEN_HEADER),
) -> None:
...
web_login_pairing_router = APIRouter(
    prefix="/api/v1/auth/web-pairings",
    tags=["网站登录配对"],
)
...
def confirm_web_login_pairing(
    payload: PairingConfirmRequest,
    pairing_id: str = ApiPath(min_length=10, max_length=100),
    _enabled: None = Depends(require_web_login_pairing_enabled),
    principal: UserPrincipal = Depends(require_current_user),
):
    try:
        return success(
            pairing_service.confirm(pairing_id, payload.verification_code, principal),
            "网站登录已确认",
        )
...
def claim_web_login_pairing(
    payload: PairingClaimRequest,
    pairing_id: str = ApiPath(min_length=10, max_length=100),
    _authorized: None = Depends(require_web_login_pairing_bff),
):
    try:
    ...
    return success(
        {
            ...
            "user": user or {"user_id": result["user_id"]},
        },
        "网站登录成功",
    )
