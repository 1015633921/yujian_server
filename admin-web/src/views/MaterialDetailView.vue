...
import { getMaterial, updateMaterial, type Material } from '@/features/materials/api'
...
async function save(event: Event) { if (!item.value || saving.value) return; const data = new FormData(event.target as HTMLFormElement); const sku = item.value.sku || {}; const payload = { id: item.value.id, skuId: sku.sku_id || '', top: item.value.top || 'bead', category: String(item.value.category || ''), series: String(item.value.series || item.value.name || ''), grade: String(item.value.grade || ''), name: item.value.name || '', price: Number(data.get('price')), cost_price: Number(data.get('cost')), stock: Number(data.get('stock')), safety_stock: Number(data.get('safety')), size: Number(sku.size_mm || item.value.size || 8), weight: Number(item.value.weight || 1), enabled: data.get('enabled') === 'true', sort_order: Number(item.value.sort_order || 0), physical_specs: item.value.physical_specs || {} }; saving.value = true; message.value = ''; try { item.value = await updateMaterial(item.value.id, payload); message.value = 'SKU 商业资料已保存。' } catch (cause) { message.value = cause instanceof Error ? cause.message : '保存失败' } finally { saving.value = false } }
